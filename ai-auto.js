...
// « Stratégies IA » que si l'analyse lui mesure au moins 80% de réussite.
// C'est aussi le seuil d'entrée du panneau « Prédit » (prédictions IA).
const MIN_STRATEGY_RATE = 80;
...
function saveProposal(proposal, origin = 'auto-local') {
  ...
  const item = {
    ...
    // règle machine (déclencheur → costume ciblé) utilisée par les prédictions IA
    rule: proposal.rule || null,
    lead: Number(proposal.suggestedLead) || (proposal.rule && proposal.rule.k) || null,
    compatibleExisting: null,
    origin,
    createdAt: new Date().toISOString(),
    active: false,
  };
  item.explanation = explain.buildExplanation(item);
  item.summary = explain.buildSummary(item);
...
function status() {
  return {
    ...
    strategies: (state.aiStrategies || []).map(explain.decorate),
