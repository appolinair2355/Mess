...
app.get('/api/state', (req, res) => {
  res.json({
    ...
    ai: {
      ...
      savedStrategies: (state.aiStrategies || []).map(explain.decorate),
    },
...
app.delete('/api/ai/strategies/:id', (req, res) => {
  ...
  persist();
  res.json({ ok: true, savedStrategies: (state.aiStrategies || []).map(explain.decorate) });
});


// --- panneau « Prédit » (stratégies IA ≥ 80% ; 100% = continu) --------------
